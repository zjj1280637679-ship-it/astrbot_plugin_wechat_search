"""SQLite fact index. WeChat remains the source of truth; this database is rebuildable."""
...
class WeChatSearchStore:
    ...
    def _canonical(account_id: str, record: dict) -> tuple:
        ...
        stable = {
            ...
            "segment_types": WeChatSearchStore._types(record.get("segment_types")),
    ...
    def upsert_many(self, account_id: str, records: Iterable[dict]) -> int:
        ...
        with self._lock, self._conn:
            ...
            # to the same group message. WeChat's per-chat message sequence is the
