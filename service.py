...
from .source import WxSourceError


class SourceBoundaryError(WxSourceError):
...
def _citation(row: dict) -> str:
    return f"wx:{row.get('group_id', '')}:{row.get('message_id', '')}"
        ...
        *,
        ...
        try:
            for _ in range(min(max(int(pages), 1), 100)):
                ...
                if rejected:
                    raise SourceBoundaryError("微信信息源返回了目标群以外的记录，已拒绝整页。")
    ...
    async def list_member_messages(
        ...
        *,
        ...
        try:
            ...
            if rejected:
                raise SourceBoundaryError("微信信息源返回了目标群或目标成员以外的记录，已拒绝整页。")
            for message in messages:
                if isinstance(message, dict):
            ...
            acquisition = {
                ...
                "cursor": "",
                "next_cursor": "",
                "warning": "微信原生信息源不可用；结果仅来自本地缓存，可能缺失。",
            }
